...
class DiagnosisService:
    ...
    #         unmatched_ids = condition_ids - facts
    ...
    #             unmatched_names = [symptoms[sid].symptom_name for sid in unmatched_ids if sid in symptoms]
    ...
    def infer(cls, selected_symptom_ids: List[int]) -> Tuple[Dict[int, dict], Dict[str, List[dict]], List[dict]]:
        ...
        # 1. Prefetch all active rules, diseases, and symptoms
        rules = RulesTable.query.filter_by(is_active=True).all() or []
        diseases = {d.id: d for d in DiseaseTable.query.all()}
        symptoms = {s.id: s for s in SymptomsTable.query.all()}

        # 2. Optimization: Prefetch ALL active rule conditions at once (Avoid N+1 queries in loop)
        all_conditions = RuleConditionsTable.query.filter_by(is_active=True).all() or []
        rule_conditions_map: Dict[int, List] = {}
        for cond in all_conditions:
            rule_conditions_map.setdefault(cond.rule_id, []).append(cond)
        ...
        for rule in rules:
            ...
            # Get prefetched conditions for this rule
            conditions = rule_conditions_map.get(rule.id, [])
            condition_ids = {c.symptom_id for c in conditions}
            if not condition_ids:
                continue
            ...
            # Pre-resolve symptom names for both matched & unmatched
            matched_names = [symptoms[sid].symptom_name for sid in matched_ids if sid in symptoms]
            unmatched_names = [symptoms[sid].symptom_name for sid in unmatched_ids if sid in symptoms]
            ...
            if matched_count > 0:
                ...
                # Initialize disease entry in conclusions
                if disease.id not in conclusions:
                    conclusions[disease.id] = {"disease": disease, "certainty": 0.0}
                
                # Guarantee rule_trace key exists for string ID
                if str_disease_id not in rule_trace:
                    rule_trace[str_disease_id] = []

                # Read current certainty BEFORE applying this rule
                prev_cf = float(conclusions[disease.id]["certainty"])

                # Combine previous CF with this rule's contribution
                new_cf = cls.combine_cfs(prev_cf, adjusted_cf)
                conclusions[disease.id]["certainty"] = new_cf
                ...
                rule_trace[str_disease_id].append({
                    ...
                    "matched": matched_names,
                    "unmatched": unmatched_names,
                    "cf_before": float(round(prev_cf, 4)),
                    "rule_cf": float(round(adjusted_cf, 4)),
                    "cf_after": float(round(new_cf, 4)),
                    "explanation": getattr(rule, "explanation", "") or ""
                })
            else:
                # Track skipped rules safely
                current_disease_cf = conclusions.get(disease.id, {}).get("certainty", 0.0)
                
                skipped_rules.append({
                    "rule_id": rule.id,
                    "disease": disease.disease_name,
                    "missing": unmatched_names,  # All required conditions were missed
                    "unmatched": unmatched_names,
                    "cf_before": float(round(current_disease_cf, 4)),
                    "rule_cf": 0.0,
                    "cf_after": float(round(current_disease_cf, 4)),
